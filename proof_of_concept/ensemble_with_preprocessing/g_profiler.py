...
# ------------------------------------------------------------------
# Helper functions (used in both full and fraction analyses)
# ------------------------------------------------------------------
...
outdir = "/project_antwerp/baseline/ensemble_preprocessing/plots"
...
results_full = gp.profile(
    organism='hsapiens',
    query=pooled_genes_full,
...
print("\n--- Raw results summary (full) ---")
print("Shape of results:", results_full.shape)

if not results_full.empty:
    significant_full = results_full[results_full['significant']]
    if significant_full.empty:
        print("No significant results for full pooled set.")
    else:
        results_full = significant_full.sort_values('p_value')
        cols = ['source', 'name', 'p_value', 'precision', 'recall', 'intersection_size', 'term_size', 'intersections']
        print(results_full[cols].to_string(index=False))

        plot_df_full = results_full.copy()
        plot_df_full = plot_df_full.dropna(subset=["p_value", "name", "source", "intersection_size", "term_size"])
        plot_df_full["neg_log10_p"] = -np.log10(plot_df_full["p_value"])
        plot_df_full["gene_ratio"] = plot_df_full["intersection_size"] / plot_df_full["term_size"]
        plot_df_full["gene_list"] = plot_df_full["intersections"].apply(parse_intersections)

        top_df_full = plot_df_full.sort_values("p_value").copy()
        top_df_full["label"] = top_df_full["name"].apply(wrap_label)

        bar_df_full = top_df_full.sort_values("neg_log10_p", ascending=True)
        ...
        bar_df_full["color"] = bar_df_full["source"].map(source_colors)

        plt.figure(figsize=(10, max(5, 0.4 * len(bar_df_full))))

        plt.barh(
            bar_df_full["label"],
            bar_df_full["neg_log10_p"],
            color=bar_df_full["color"],
            edgecolor="black",
            linewidth=0.6
        )
        ...
        plt.ylabel("")
        plt.title("Enriched pathways — full pooled set (GO:BP and Reactome)", fontsize=13, weight="bold")
        ...
        plt.tight_layout()
        plt.savefig(f"{outdir}/barplot_full.png")
        plt.close()

        print(f"\nSaved full pooled set enrichment plot to: {outdir}")
...
fraction = 0.7
fractions = [0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65]
